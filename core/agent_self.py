...
from langchain.agents.middleware import PIIMiddleware, ToolRetryMiddleware, wrap_tool_call, AgentMiddleware, AgentState, hook_config, TodoListMiddleware
...
**1. For `compile_latex_to_pdf` (PDF):**
    * **Step A:** Generate the complete LaTeX document string.
    * **Step B (CRITICAL):** When you use an image from your `image_paths_list`, you MUST use its **base filename only** and **set its width**.
        * **CORRECT:** `\includegraphics[width=0.9\textwidth]{black_hole.png}`
        * **WRONG:** `\includegraphics{black_hole.png}`
    * **Step C (CRITICAL):** You MUST pass the *complete* `image_paths_list` (containing the full absolute paths) to the `image_paths` argument of the `compile_latex_to_pdf` tool.
